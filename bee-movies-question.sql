...
SELECT EXTRACT(MONTH FROM date_published) AS month_num, COUNT(title) as number_of_movies
FROM movie 
GROUP BY month_num
ORDER BY month_num;
...
-- Type your code below:
SELECT g.genre, COUNT(title) AS highest_number_of_movies_produced_overall 
FROM genre g
JOIN movie m ON g.movie_id = m.id
GROUP BY g.genre
ORDER BY highest_number_of_movies_produced_overall DESC
LIMIT 1; 
...
JOIN movie m ON single_genre_movies.movie_id = m.id;
...
WHERE 
    m.country = 'USA'
    AND TO_CHAR(m.date_published, 'YYYY-MM') = '2017-03'
...
-- Type your code below:
SELECT m.country, SUM(r.total_votes) AS total_number_of_votes
FROM movie m
JOIN ratings r ON m.id = r.movie_id
...
SELECT 
    ...
    ROUND(SUM(AVG(m.duration)) OVER (
        ORDER BY g.genre
    ), 2) AS running_total_duration,
    ROUND(AVG(AVG(m.duration)) OVER (
        ORDER BY g.genre
        ...
        ORDER BY 
            CASE
                WHEN m.worlwide_gross_income LIKE 'INR%' 
                    THEN CAST(REPLACE(m.worlwide_gross_income, 'INR ', '') AS BIGINT)
                WHEN m.worlwide_gross_income LIKE '$%' 
                    THEN CAST(REPLACE(m.worlwide_gross_income, '$', '') AS BIGINT)
                ELSE NULL
            END DESC
    ) AS movie_rank
FROM genre g
JOIN movie m 
    ON m.id = g.movie_id
WHERE m.worlwide_gross_income IS NOT NULL AND 
g.genre IN (
    SELECT genre
    FROM genre
    GROUP BY genre
...
-- In the role_mapping table, the category does not have 'director'.
-- Therefore, the result will give an empty table.
-- Select * from names;
